# CLAUDE.md - Dominion-Transformer Project Guide
...
1. **Throne Room PlayNonTerminal resolver incomplete**
...
2. **Ascending index constraint logic**
...
3. **Macro action with negative-draw cards**
...
2. **Reduce Action Space**
   - Current: 4096 action space
   - Optimal: ~250 actions
   - Reduces policy head output layer size

3. **Improve Macro Actions**
...
### Why FIFO Effect Queues?

**Dominion's card interactions must resolve in a specific order.** A FIFO queue ensures:
- Clear execution order (no ambiguity about which card resolves first)
- Sequential choice resolution (one effect completes before next begins)
- Compatibility with OpenSpiel's turn-based semantics

**Alternative considered:** Parallel effect resolution (e.g., multiple attacks simultaneously)
**Rejected because:** Dominion rules specify sequential resolution and card triggers are order-dependent.

### Why Stateless Card Specs?

**Card definitions are singletons; runtime state is in EffectNodes and PlayerState.**

**Benefits:**
- Memory efficient (one copy of card specs per process)
- Simplifies serialization (no need to serialize card objects)
- Clear separation: specs are rules, state is the game instance

### Why Polymorphic EffectNodes?

**Each card with unique behavior gets its own EffectNode subclass.**

**Benefits:**
- Encapsulation: Each effect knows how to process its own logic
- Type safety: Compiler ensures proper implementation
- Extensibility: New cards = new node classes

**Alternative considered:** Single EffectNode with a type discriminator
**Rejected because:** Would require massive switch statements and lose compile-time safety.

### Why CRTP for clone()?

**Curiously Recurring Template Pattern eliminates boilerplate clone() implementations.**

**Before CRTP (7 lines per class):**
```cpp
std::unique_ptr<EffectNode> clone() const override {
...
**After CRTP (0 lines - automatic):**
```cpp
class CellarEffectNode : public CloneableEffectNode<CellarEffectNode> {
  // clone() automatically implemented by CRTP base
};
```

**Impact:** Eliminated 63 lines of boilerplate across 7 effect node classes.

### Why EffectNodeFactory?

**Centralizes effect node creation for consistency and maintainability.**

**Benefits:**
- Single source of truth for node construction
- Simplifies JSON deserialization (already implemented in `EffectNodeFromStruct`)
- Reduces duplication across card implementations
- Makes it easier to add logging, instrumentation, or validation
...
1. **LegalActions() computation** - Called O(MCTS visits) times per tree
   - Currently recomputes everything on each call
   - Recommended: Implement caching with dirty flag

2. **Hand iteration** - `array<int, 33>` requires O(N) scans
   - Recommended: Use sparse `map<CardName, int>` for large hands
   - Negligible impact for Dominion (hands ≤ 20 cards)

3. **Action space size** - 4096 actions vs ~200 actual
   - Increases policy network output layer size
   - Recommended: Reduce to 250 and use action masking
...
| High | Legal action caching | 2-5x speedup | Low |
| High | Reduce action space | 16x smaller policy head | Low |
| Medium | Complete migration to helper methods | Maintainability | Low |
| Medium | Extract LegalActions() logic | Readability | Medium |
| Low | Sparse hand representation | Negligible (small hands) | Medium |
...
**Author:** Generated for AI Assistant Context
